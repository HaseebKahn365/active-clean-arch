              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    _DurationText(activityId: activity.id),
                  ],
                ),
              ),
      ...
      children: [
        if (activity.status == ActivityStatus.running)
          _ActionButton(
            ...
            color: Colors.orange, // Use a fixed semantic meaning or secondary
            onTap: () => context.read<ActivityController>().pauseActivity(activity.id),
          )
        else
          _ActionButton(
            icon: Icons.play_arrow_rounded,
            color: colorScheme.primary,
            onTap: () => context.read<ActivityController>().startActivity(activity.id),
          ),
        const SizedBox(width: 8),
        _ActionButton(
          icon: Icons.done_all_rounded,
          color: colorScheme.secondary,
          onTap: () => context.read<ActivityController>().completeActivity(activity.id),
        ),
        const SizedBox(width: 8),
        _ActionButton(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              context.read<ActivityController>().deleteActivity(activity.id);
              Navigator.pop(ctx);
            },
